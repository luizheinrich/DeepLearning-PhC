...
RandYCDDS2 = A + (B-(A)) .* rand(1,1);
RandXCDDS2 = A + (B-(A)) .* rand(1,1);
RandRCDDS2 = A + (B-(A)) .* rand(1,1);
...
RandIntYCDDS2 = randi([-5 5],1,1);
RandIntXCDDS2 = randi([-5 5],1,1);
RandIntRCDDS2 = randi([-5 5],1,1);
RandIntYSSF = randi([-5 5],1,1);
RandIntRSSF = randi([-5 5],1,1);


% Pesos das variações dos parâmetros
PesoYF   = -3.667066119865971 + RandYF*0.3;
PesoRF   = -1.297298198201521 + RandRF*0.3;

PesoYC   = -2.197257646046233 + RandYC*0.3;
PesoXC   = 3.978515132793933 + RandXC*0.3;
PesoRC   = 0; %7.427673795081759 + RandRC*0.3;

PesoYCS  = 0; %2.081436020271390;% + RandYCS*0.3;
PesoRCS  = 0; %3.051044244194831 + RandRCS*0.3;

PesoYCDS = 3.484590328263255 + RandYCDS*0.3;
PesoXCDS = 0.723802539421964 + RandXCDS*0.3;
PesoRCDS = 0.801487420774726 + RandRCDS*0.05;

PesoYCDDS = 4.641786781364708 + RandYCDDS*0.3;
PesoXCDDS = -3.092579617862128 + RandXCDDS*0.3;
PesoRCDDS = 0.007930066433469 + RandRCDDS*0.05;

PesoYCDDI = 1.605174231559336 + RandYCDDI*0.3;
PesoXCDDI = 0.494946345883838 + RandXCDDI*0.3;
PesoRCDDI = -0.815609525434183 + RandRCDDI*0.05;

PesoYCDDI1 = -5.090827790034036 + RandYCDDI1*0.3;
PesoXCDDI1 = 5.550544377974270 + RandXCDDI1*0.3;
PesoRCDDI1 = 0.936584109128201 + RandRCDDI1*0.05;

PesoYCDDII1 = -9.855835512651701 + RandYCDDII1*0.5;
PesoXCDDII1 = 0.664441885573726 + RandXCDDII1*0.5;
PesoRCDDII1 = -1.306343659293907 + RandRCDDII1*0.1;

PesoYCDDS2 = -0.389548991072690 + RandYCDDS2*0.4;
PesoXCDDS2 = -0.174607895765868 + RandXCDDS2*0.4;
PesoRCDDS2 = -0.121085598130313 + RandRCDDS2*0.1;

PesoYSSF = 0.762480444911427 + RandYSSF*0.4;
PesoRSSF = -1.699923254543977 + RandRSSF*0.4;
...
YCDDS2 = num2str(PesoYCDDS2,'%.15f');
XCDDS2 = num2str(PesoXCDDS2,'%.15f');
RCDDS2 = num2str(PesoRCDDS2,'%.15f');
...
A1 = {'YF' 'RF' 'YC' 'XC' 'RC' 'YCS' 'RCS' 'YCDS' 'XCDS' 'RCDS' 'YCDDS'...
      ...
      'YCDDII1' 'XCDDII1' 'RCDDII1' 'YCDDS2' 'XCDDS2' 'RCDDS2' 'YSSF' 'RSSF'};
